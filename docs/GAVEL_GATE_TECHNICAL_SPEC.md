...
| `POST /v1/submissions/:publicId/relay` | Exact `base_sender` session equal to payer/signed sender; mounted only when a relayer is configured | Accept `{ authorization: { signature } }` and NOTHING else — no `to`, `data`, or `value`. Resolve the owner-bound quote from Gate's own record, re-verify its signature, deployment identity, payer, and expiry, verify the EIP-3009 authorization recovers to that payer, rebuild the `settle` calldata, re-run the shared prepared-settlement guard, and broadcast `{ to, data, value: 0 }` with a gas-only relayer wallet that never holds or moves USDC and is never the payer. Return `{ txHash, chainId, relayer }`. One quote broadcasts once; a repeat returns the same hash. The hash is a settlement hint only: it does not record settlement, does not create an inbox, and is not acceptance. |

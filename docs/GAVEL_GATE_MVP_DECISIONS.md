...
- [x] Nouns exposes only tested native lifecycle mappings. The adapter consumes upstream canonical labeled states: `ACTIVE` -> `VOTING`; raw Governor numeric codes are normalized upstream, and numeric `1` plus string `"1"` fail closed to `CLOSED`. Generic vocabulary retains `PRE_VOTE`, but Nouns policy and quote issuance are `VOTING`-only until a real actionable mapping is separately frozen, implemented, and tested.
...
- [x] `submission_hash` is ethers `keccak256(toUtf8Bytes(JSON.stringify(array)))` over exactly `["gavel-gate-submission-v1", payer, voter, dao, proposalId, stage, position, pitch, disclosures, evidenceUrls]` in that order. Addresses use ethers `getAddress`; proposal ID is canonical unsigned decimal with no leading-zero variant; pitch/disclosures retain exact JavaScript string bytes after JSON escaping with no trimming or whitespace/newline normalization; evidence URL order is significant. All later code imports `hashSubmission` from `@gavel/gate` and never reimplements it.
...
| --- | --- | --- |
| Lifecycle | Tested Nouns native mapping; VOTING supported | Nouns PRE_VOTE mapping expansion and more DAOs/states |

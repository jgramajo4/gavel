...
- [x] Nouns exposes only tested native lifecycle mappings. Current MVP mapping is canonical `ACTIVE` -> `VOTING`; all other/unknown states fail closed to `CLOSED`. Do not advertise `PRE_VOTE` until a real native mapping is implemented and tested.
...
- [x] Constrained CommonMark AST only; no HTML, images, embeds, scripts, Mermaid, previews, uploads, or evidence fetching.

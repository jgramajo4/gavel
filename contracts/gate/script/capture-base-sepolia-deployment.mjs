...
function bytes32(value, label) {
  if (!BYTES32.test(String(value ?? ""))) fail(`${label} must be bytes32`);
...
function validateStructure(artifact) {
  ...
  uint(artifact.token.deployment.block, "token.deployment.block");
  ...
  uint(artifact.splitter.deployment.block, "splitter.deployment.block");
...
function capture(args, rpc) {
  ...
  if (process.env.SYNTHETIC_DEPLOYMENT_TEST !== "yes"
      && (isAbsolute(relative(OUTPUT_ROOT, output)) || relative(OUTPUT_ROOT, output).startsWith("..") || !output.endsWith(".json"))) {

...
1. Complete `runtime.md`, then run the read-only chain briefing with the voter address:

   ```bash
   node gavel/nouns-dao/scripts/daily_briefing.js --voter=0xVoterAddress --limit=10 --since-hours=24
   ```
...
5. Run the read-only preparation gate from the repository root:
   ...
   ```bash
   node gavel/bin/gavel.js prepare-vote \

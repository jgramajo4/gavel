...
function parseArgs(argv) {
  const args = { blocks: 5_000, latency: 20, baseline: null, noise: 6 };
...
function createProbe(chain, latencyMs) {
  ...
  const provider = new ProbeProvider("http://probe.invalid", 8453, { staticNetwork: true });
  const client = {
...
async function run(label, factory, chain, { from, through, latency }) {
  const { client, stats } = createProbe(chain, latency);
  const adapter = factory({ client, chainId: 8453, splitter: SPLITTER, maxBlockRange: 100_000, rpcTimeoutMs: 60_000 });
...
function report(result) {
  ...
  process.stdout.write(`  blocks scanned   : ${result.blocks}\n`);
...
async function main() {
  ...
  for (const [name, list] of [["sparse (no settlements)", []], ["one settlement", settlements]]) {
    ...
    process.stdout.write(`\n=== ${name} ===\n`);
    if (args.baseline) {
      const { createBaseSettlementAdapter } = require(args.baseline);
      report(await run("BEFORE (per-block receipts, sequential)", createBaseSettlementAdapter, chain,
    ...
    report(await run("AFTER  (eth_getLogs + bloom-gated receipts, batched headers)", createBaseSettlementAdapter, chain,
      { from, through, latency: args.latency }));
  }
}
